...
setup_cron_job() {
    ...
    case $choice in
        ...
        2)
            ...
            # Create cron job and append to existing crontab
            cron_file="/root/cronjob"
            new_cron="*/$interval * * * * systemctl restart $service_name"
            if crontab -l > /dev/null 2>&1; then
                # Append new cron job to existing crontab
                crontab -l > "$cron_file"
                if ! grep -q "$service_name" "$cron_file"; then
                    echo "$new_cron" >> "$cron_file"
                    crontab "$cron_file"
                    colorize green "Cron job set to restart $config_name every $interval minutes."
                else
                    colorize yellow "Cron job for $config_name already exists. Skipping addition."
                fi
            else
